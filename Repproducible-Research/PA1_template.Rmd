...
The 5-minute interval which contains the maximum number of steps,  on average across all the days in the dataset, is `r maxInterval`.
...
Total number of missing values in the dataset is `r noOfNASteps`.
...
Mean of total number of steps taken per day is:  `r newMeanSteps`.  
Median of total number of steps taken per day is: `r newMedianSteps`.
...
The plot shows that on average the number of steps are mostly below 160 for each interval on weekdays, while on weekends on average the number of steps are mostly below 120 per interval, but there is a spike of `r maxStepsFormatted` at interval `r maxNewInterval`.This is the same interval as we observed earlier which contains maximum number of steps,  on average across all the days in the original dataset. This means this person in general is physically more active at 8:35am on weekends. 
